...
export default function Home(){
 ...
 if(screen==="home")return <main className="v8Home" data-build="V8.1-HOME-OFICIAL"><section className="v8Hero" aria-label="Tela principal de As Aventuras de Geovana"><img className="v8HeroImage" src="/home-v8-pudim.png?v=81" alt="As Aventuras de Geovana com Geovana e Pudim" draggable={false}/><nav className="v8Hotspots" aria-label="Menu principal"><button className="v8Play" onClick={()=>setScreen(profile.name?"map":"profile")} aria-label={profile.name?"Continuar aventura":"Jogar"}/><button className="v8Worlds" onClick={()=>setScreen("map")} aria-label="Mundos"/><button className="v8Ranking" onClick={()=>setScreen("achievements")} aria-label="Ranking"/><button className="v8Learn" onClick={()=>setScreen("settings")} aria-label="Aprender"/></nav></section></main>;
