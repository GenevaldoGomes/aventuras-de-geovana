...
export default function Home(){
 ...
 return <main>{top}
 {screen==="home"&&<section className="heroArt" aria-label="Tela principal de As Aventuras de Geovana"><img className="heroArtImage" src="/home-pudim.jpg" alt="As Aventuras de Geovana com Geovana e seu amigo Pudim"/><div className="heroHotspots" aria-label="Menu principal"><button className="hotPlay" onClick={()=>setScreen(profile.name?"map":"profile")} aria-label={profile.name?"Continuar jogo":"Jogar"}></button><button className="hotWorlds" onClick={()=>setScreen("map")} aria-label="Mundos"></button><button className="hotRanking" onClick={()=>setScreen("achievements")} aria-label="Ranking e conquistas"></button><button className="hotLearn" onClick={()=>setScreen("settings")} aria-label="Aprender e configurações"></button></div></section>}
