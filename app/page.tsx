...
type MascotMood="idle"|"talk"|"correct"|"wrong"|"happy";
...
function Pudin({mood="idle",small=false}:{mood?:MascotMood;small?:boolean}){
 return <div className={`pudin ${mood} ${small?"small":""}`} aria-label="Pudin, mascote do jogo"><span className="pudinBubble">{mood==="correct"?"Great! ⭐":mood==="wrong"?"Try again! 💛":mood==="talk"?"Listen! 🔊":mood==="happy"?"Let’s go! 🚀":"Hi! 👋"}</span><img src="/pudin-mascot.png" alt="Pudin, gato mascote"/></div>
}
...
export default function Home(){
 ...
 const answer=(value:string)=>{
  ...
  if(ok){setMascotMood("correct");setTimeout(()=>setMascotMood("idle"),1600);setFeedback(["Great job!","Excellent!","Amazing!","You got it!"][(q+world)%4]);setProfile(p=>({...p,correct:p.correct+1,streak:p.streak+1,stars:p.stars+1,coins:p.coins+5,xp:p.xp+20,themeScore:{...p.themeScore,[theme]:{correct:(p.themeScore[theme]?.correct||0)+1,wrong:p.themeScore[theme]?.wrong||0}}}));speak("Great job!")}
  else{setMascotMood("wrong");setTimeout(()=>setMascotMood("idle"),1500);const n=tries+1;setTries(n);setFeedback(n>=2?`💡 ${item.tip}`:"Almost! Try again!");setProfile(p=>({...p,wrong:p.wrong+1,streak:0,themeScore:{...p.themeScore,[theme]:{correct:p.themeScore[theme]?.correct||0,wrong:(p.themeScore[theme]?.wrong||0)+1}}}));speak("Good try! Let's try one more time!");setTimeout(()=>setFeedback(""),n>=2?2800:1400)}
 };
