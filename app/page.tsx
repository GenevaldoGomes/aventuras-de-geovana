...
export default function Home(){
 const[screen,setScreen]=useState<Screen>("home"),[profile,setProfile]=useState<Profile>(fresh),[loaded,setLoaded]=useState(false),[world,setWorld]=useState(0),[q,setQ]=useState(0),[feedback,setFeedback]=useState(""),[tries,setTries]=useState(0),[parentPass,setParentPass]=useState(""),[unlocked,setUnlocked]=useState(false),[sound,setSound]=useState(true);
 ...
 const speak=(text:string)=>{if(!sound)return;speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang="en-US";u.rate=profile.age<=8?.7:.82;speechSynthesis.speak(u)};
 ...
 const answer=(value:string)=>{
  ...
  if(ok){setFeedback(["Great job!","Excellent!","Amazing!","You got it!"][(q+world)%4]);setProfile(p=>({...p,correct:p.correct+1,streak:p.streak+1,stars:p.stars+1,coins:p.coins+5,xp:p.xp+20,themeScore:{...p.themeScore,[theme]:{correct:(p.themeScore[theme]?.correct||0)+1,wrong:p.themeScore[theme]?.wrong||0}}}));speak("Great job!")}
  else{const n=tries+1;setTries(n);setFeedback(n>=2?`💡 ${item.tip}`:"Almost! Try again!");setProfile(p=>({...p,wrong:p.wrong+1,streak:0,themeScore:{...p.themeScore,[theme]:{correct:p.themeScore[theme]?.correct||0,wrong:(p.themeScore[theme]?.wrong||0)+1}}}));speak("Good try! Let's try one more time!");setTimeout(()=>setFeedback(""),n>=2?2800:1400)}
 };
 ...
 return <main>{top}
 {screen==="home"&&<section className="hero"><div className="heroCopy"><p className="eyebrow">A magical English journey</p><h1>AS AVENTURAS<br/><em>DE GEOVANA</em></h1><h2>GEOVANA'S ADVENTURES</h2><p>Aprenda inglês enquanto vive grandes aventuras!<br/><b>Learn English While Having Great Adventures!</b></p></div><div className="mascot"><img src="/geovana.png" alt="Geovana sorrindo e acenando"/><span className="leaf">🌿</span></div><div className="homeBtns"><button className="primary" onClick={()=>setScreen(profile.name?"map":"profile")}>▶ {profile.name?"CONTINUAR — CONTINUE":"JOGAR — PLAY"}</button><button onClick={()=>setScreen("map")}>🗺️ MAPA — ADVENTURE MAP</button><button onClick={()=>setScreen("achievements")}>🏆 CONQUISTAS — ACHIEVEMENTS</button><button onClick={()=>setScreen("settings")}>⚙️ CONFIGURAÇÕES — SETTINGS</button></div></section>}
